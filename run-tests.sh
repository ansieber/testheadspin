...
executeTests(){
	...
	if [ "$(uname)" == "Darwin" ]; then
		...
		mvn clean test -DexecutionType=serverside -Denv=SUT_DEFAULT -DsuiteXmlFile=src/test/resources/testSuite/SuiteTestNgIOS.xml
	elif [ "$(expr substr $(uname -s) 1 5)" == "Linux" ]; then
	    echo "Running Android Tests..."
		mvn clean test -DexecutionType=serverside -Denv=SUT_DEFAULT -DsuiteXmlFile=src/test/resources/testSuite/SuiteTestNgAndroid.xml
		#SuiteTestNgAndroid
		#SuiteTestNgWebApp
	fi
