...
public class BaseTest {
	...
	private static boolean initCleanAndroidPackageData = true;
		...
		try {
			...
			switch (expectedSUT) {
				...
				case APPIUM_ANDROID_APPLICATION:
					if (initCleanAndroidPackageData) {
						clearAndroidPackageData(propSUT.getProperty("android.appPackage"));
//						initCleanAndroidPackageData = false; // execute only one time		
					}					
					Common.sleep(waitStartAppium);
    ...
    private void clearAndroidPackageData(String packageName) throws Exception {
    	LOGGER.info("adb shell pm clear "+packageName);
    	Runtime runtime = Runtime.getRuntime();
		...
		try {
	    	...
	    	String output = IOUtils.toString(process.getInputStream(),"UTF-8");
	    	String errorOutput = IOUtils.toString(process.getErrorStream(),"UTF-8");    	
	    	LOGGER.info("adb code retour "+output);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
